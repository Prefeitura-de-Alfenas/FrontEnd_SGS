...
import {
  GetFamiliares,
  changeReponsavelFamiliar,
} from "@/app/api/familiares/routes";
...
const TableFamiliares = ({ usuario, responsavelId }: TableFamiliaresProps) => {
  ...
  const mutation = useMutation({
    mutationFn: (id: string) => {
      return changeReponsavelFamiliar(usuario, id).then((response) => response);
    },
    onError: (error) => {
      toast({
        variant: "destructive",
    ...
    <div className="flex flex-col ">
      <div className="flex items-start justify-start">
        <Button className="ms-1 mt-4 mb-4 text-white font-bold">
          <Link href={`/familiares/novofamiliar/${responsavelId}`}>
            Novo Familiar{" "}
          </Link>
        </Button>
      </div>
        ...
        <TableBody>
          {data?.map((pessoa: PessoaI) => (
            <TableRow key={pessoa.id}>
              ...
              <TableCell>
                <Link
                  href={`/familiares/editarfamiliar/${pessoa.id}/responsavel/${responsavelId}`}
                >
                  <FileEdit />
                </Link>
              </TableCell>
              <TableCell
                ...
                onClick={() => handleChangeResponsavel(pessoa.id)}
              >
                <UserRoundCog />
              </TableCell>
            </TableRow>
