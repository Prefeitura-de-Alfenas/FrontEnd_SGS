...
function AddPessoa({ usuario, id, refetch }: DeleteSoftPessoa) {
  ...
  const mutation = useMutation({
    mutationFn: ({ id }: { id: string }) => {
      return ChangeStatus(usuario, id).then((response) => response);
    },
    onError: (error) => {
      toast({
        title: error.message,
  ...
  return (
    <Dialog>
      <DialogTrigger>
        <UserRoundPlus color="blue" />
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Você está abusolutamente certo que quer Ativar essa familia?
          </DialogTitle>
          <DialogDescription>
            Se voce ativar essa familia ela estára habita há receber beneficios
            ou qual quer outro tipo de atendimento
          </DialogDescription>
        </DialogHeader>
        <div className="p-2">
          <DialogClose asChild>
