...
import { GetCepViaCep, UpdatePessoa } from "@/app/api/pessoas/routes";
...
function EditarEquipamento({equipamentoId}:EditEquipamentosProps ) {
  ...
  const { handleSubmit,register,setValue,formState:{errors}} = useForm<FormData>({
    ...
    useEffect(() => {
      ...
      setValue("responsavel", data ? data.responsavel :'',{ shouldValidate: true });
      setValue("sobre", data ? data.responsavel :'',{ shouldValidate: true });
