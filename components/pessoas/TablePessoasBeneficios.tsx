...
const TablePessoasBeneficios = ({
  usuario,
  pessoaId,
}: TablePessoasBeneficiosProps) => {
  ...
  const { data: dataBeneficioario, isPending: isPendingBeneficioario } =
    useQuery({
  ...
  const mutation = useMutation({
    mutationFn: ({ beneficioId, pessoaId }: BeneficioOnPessoasI) => {
      return ChangeBeneficioPessoa(usuario, beneficioId, pessoaId).then(
        (response) => response
      );
    },
    onError: (error) => {
        ...
        <TableBody>
          {data?.map((beneficio: BeneficiosI) => (
            <TableRow key={beneficio.id}>
              ...
              <TableCell>
                {isPendingBeneficioario ? "" : dataBeneficioario.nome}
              </TableCell>
