...
import { EquipamentoI } from "@/interfaces/equipamento/interface";
  ...
  function UpdateUsuario({ params }: { params: { userid: string } }) {
    const { toast } = useToast()
    const router = useRouter();
    
    const {data,isPending,isError,isSuccess } = useQuery({
      queryKey:['usuario',params.userid],
      queryFn:() => GetUsuarioById(params.userid)
    })
    ...
    const mutation = useMutation({
      mutationFn: (dataMuation:FormData) => {
        return     UpUsuario(params.userid,dataMuation)
